...
// alteração que da um atraso na bola e velocidade ao barco
var cooldown = 0
var PirateSpeed= -1.2
...
function draw() {
  ...
  //condição para atrasar o tiro do canhão
...
function showBoats() {
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      Matter.Body.setVelocity(boats[i].body, {
        //alterar a velocidade do pirata
        // x: -0.9,
        x:PirateSpeed,
        y: 0
      });
  ...
  // comando para fazer aparecer a cada intervalo de 160
...
function keyReleased() {
  //alteração no if
  if (keyCode === DOWN_ARROW && cooldown<=1 && !isGameOver) {
    cannonExplosion.play();
    cannonExplosion.setVolume(0.05)
    balls[balls.length - 1].shoot();
    // acrescentando o atraso
    cooldown= 50
  }
}
